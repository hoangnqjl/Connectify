...
public class ElectronicController {
    ...
    // Cập nhật chỉ quantity của sản phẩm (Frontend yêu cầu)
    ...
    public ResponseEntity<?> updateProductQuantity(
        ...
        try {
            ...
            if (quantityObj == null) {
                ...
                response.put("message", "Thiếu thông tin quantity trong request");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
            }
            ...
            try {
                ...
                if (newQuantity < 0) {
                    response.put("success", false);
                    response.put("error", "Invalid quantity");
                    response.put("message", "Số lượng không thể âm");
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
                }
            } catch (NumberFormatException e) {
                response.put("success", false);
                response.put("error", "Invalid quantity format");
                response.put("message", "Định dạng số lượng không hợp lệ");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
            }
            ...
            if (newQuantity > 0) {
                existing.setStatus("instock");
            } else {
                existing.setStatus("outofstock");
            }

            electronicService.updateElectronic(existing);

            // Trả về response theo format frontend mong đợi
            ...
            response.put("message", "Lỗi khi cập nhật quantity: " + e.getMessage());
