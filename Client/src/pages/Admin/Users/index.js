                        ...
                        <tr key={user.userId}>
                            <td>
                                <img
                                    ...
                                    // onError={(e) => {
                                    //     e.target.onerror = null; // Ngăn vòng lặp lỗi nếu fallback cũng lỗi
                ...
                <form onSubmit={handleSubmit} className={cx("modal-form")}>
                    ...
                    <div className={cx("form-group")}>
                        ...
                        <select
                            ...
                            disabled
                        >
                            <option value="user">User (Default từ /auth/signup)</option>
                        </select>
                        <small style={{color: '#6c757d', fontSize: '12px', marginTop: '4px', display: 'block'}}>
                            Endpoint /auth/signup chỉ tạo user thường. Để tạo admin cần endpoint riêng.
                        </small>
                    </div>
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                onSuccess?.(); // Gọi lại dữ liệu
                onClose();     // Đóng modal
            } else {
                alert('❌ Lỗi: ' + result.message);
            }
        } catch (error) {
            alert('⚠️ Có lỗi xảy ra khi cập nhật user: ' + error.message);
        } finally {
            setLoading(false);
        }
    };
