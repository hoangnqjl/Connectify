...
public class UserController {
    ...
    public ResponseEntity<?> getUsersWithStats(@RequestHeader("Authorization") String authHeader) {
        // Kiểm tra quyền admin
        if (!authorizationService.hasAdminAccess(authHeader)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Collections.singletonMap("error", "Bạn không có quyền truy cập tài nguyên này"));
    ...
    public ResponseEntity<?> getUserStatistics(@RequestHeader("Authorization") String authHeader) {
        ...
        try {
            ...
            // Mock data cho newUsersThisMonth và activeUsers
            long newUsersThisMonth = Math.min(8, totalUsers);
            long activeUsers = Math.min(totalUsers - 2, totalUsers);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Collections.singletonMap("error", "Lỗi khi lấy thống kê người dùng: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> testAdmin() {
        Map<String, Object> response = new HashMap<>();
        response.put("message", "Admin endpoint test successful!");
