...
public class AdminProductController {
    ...
    public ResponseEntity<?> getStatisticsByCategory(@RequestHeader("Authorization") String authHeader) {
        ...
        Map<String, Object> categoryStats = allProducts.stream()
                .collect(Collectors.groupingBy(
                    p -> p.getCategory() != null ? p.getCategory().getCat_name() : "Không có danh mục",
                    Collectors.collectingAndThen(
                        Collectors.toList(),
                        products -> {
                            Map<String, Object> stats = new HashMap<>();
                            stats.put("count", products.size());
                            stats.put("cat_id", products.isEmpty() ? null :
                                (products.get(0).getCategory() != null ? products.get(0).getCategory().getCat_id() : null));
                            return stats;
                        }
                    )
                ));
    ...
    public ResponseEntity<?> getStatisticsByBrand(@RequestHeader("Authorization") String authHeader) {
        ...
        Map<String, Object> brandStats = allProducts.stream()
                .collect(Collectors.groupingBy(
                    p -> p.getBrand() != null ? p.getBrand().getBrand_name() : "Không có thương hiệu",
                    Collectors.collectingAndThen(
                        Collectors.toList(),
                        products -> {
                            Map<String, Object> stats = new HashMap<>();
                            stats.put("count", products.size());
                            stats.put("brand_id", products.isEmpty() ? null :
                                (products.get(0).getBrand() != null ? products.get(0).getBrand().getBrand_id() : null));
                            return stats;
                        }
                    )
                ));
    ...
    public ResponseEntity<?> searchProducts(
        ...
        List<Electronic> filteredProducts = allProducts.stream()
                .filter(p -> {
                    ...
                    if (categoryId != null) {
                        matches = matches && (p.getCategory() != null && p.getCategory().getCat_id() == categoryId);
                    }

                    if (brandId != null && !brandId.trim().isEmpty()) {
                        matches = matches && (p.getBrand() != null && brandId.equals(p.getBrand().getBrand_id()));
                    }
