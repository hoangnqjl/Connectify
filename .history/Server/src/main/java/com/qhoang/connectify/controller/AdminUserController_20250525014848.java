...
public class AdminUserController {
    ...
    public ResponseEntity<?> updateUserRole(
        ...
        // Kiểm tra role hợp lệ (chỉ có admin và user)
        if (!role.equals("admin") && !role.equals("user")) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Collections.singletonMap("error", "Quyền không hợp lệ. Chỉ có thể là 'admin' hoặc 'user'"));
        }
    ...
    /**
     * Test endpoint - Thống kê user không cần authentication (chỉ để test)
     */
    @GetMapping("/test-statistics")
    public ResponseEntity<?> getTestUserStatistics() {
        try {
            List<User> allUsers = userService.getAllUsers();
            long totalUsers = allUsers.size();
            long adminCount = allUsers.stream().filter(u -> "admin".equals(u.getType())).count();
