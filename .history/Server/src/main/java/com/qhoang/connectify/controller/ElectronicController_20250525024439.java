...
public class ElectronicController {
    ...
    public ResponseEntity<?> updateElectronic(
        ...
        if (!authorizationService.hasAdminAccess(authHeader)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Collections.singletonMap("error", "Bạn không có quyền sửa sản phẩm"));
        }
    ...
    // ADMIN APIs - Product Statistics
    @GetMapping("/admin-product-statistics")
    public ResponseEntity<?> getProductStatistics(@RequestHeader(value = "Authorization", required = false) String authHeader) {
        // Kiểm tra quyền admin
        if (authHeader == null || !authorizationService.hasAdminAccess(authHeader)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
        ...
        try {
            ...
            long activeProducts = allProducts.stream().filter(p -> p.getQuantity() > 0).count();
            long inactiveProducts = allProducts.stream().filter(p -> p.getQuantity() == 0).count();
            long totalStock = allProducts.stream().mapToLong(Electronic::getQuantity).sum();
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Collections.singletonMap("error", "Lỗi khi lấy thống kê sản phẩm: " + e.getMessage()));
        }
    }

    // ADMIN APIs - Low Stock Products
    ...
    public ResponseEntity<?> getLowStockProducts(
        ...
        try {
            ...
            // Lọc sản phẩm có quantity <= threshold
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Collections.singletonMap("error", "Lỗi khi lấy sản phẩm sắp hết hàng: " + e.getMessage()));
        }
    }
