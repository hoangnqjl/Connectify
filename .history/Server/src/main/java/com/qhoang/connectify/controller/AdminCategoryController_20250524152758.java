...
public class AdminCategoryController {
    ...
    public ResponseEntity<?> deleteCategory(
        ...
        List<Electronic> productsInCategory = electronicService.getAllElectronics().stream()
                .filter(p -> p.getCategory() != null && p.getCategory().getCat_id() == id)
        ...
        List<Map<String, Object>> categoryStats = allCategories.stream()
                .map(category -> {
                    List<Electronic> productsInCategory = allProducts.stream()
                            .filter(p -> p.getCategory() != null && p.getCategory().getCat_id() == category.getCat_id())
                            .collect(Collectors.toList());

                    Map<String, Object> stats = new HashMap<>();
                    stats.put("categoryId", category.getCat_id());
                    stats.put("categoryName", category.getCat_name());
